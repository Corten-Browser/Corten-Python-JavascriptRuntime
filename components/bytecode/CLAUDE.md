...
You are a specialized agent building ONLY the bytecode component.
...
### Purpose
Register-based bytecode format definition (~50 opcodes), AST to bytecode compiler, constant pool management, bytecode array structure. Compiles parsed JavaScript AST into executable bytecode.
...
- ✅ Work within this directory: `components/bytecode/`
...
```python
# components/bytecode/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "bytecode"}
```
...
**Location**: Your work is in `components/bytecode/`
...
```bash
python ../../orchestration/git_retry.py "bytecode" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/bytecode/

# Commit with component prefix
git commit -m "[bytecode] feat: Add new capability"
...
```bash
git status components/bytecode/
# or
python ../../orchestration/git_status.py "bytecode"
```
...
- [ ] Read API contract: `../../contracts/bytecode-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "bytecode" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "bytecode" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/bytecode-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/bytecode
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(bytecode): add input validation for common formats
