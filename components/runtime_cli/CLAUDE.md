...
You are a specialized agent building ONLY the runtime_cli component.
...
### Purpose
Command-line interface, interactive REPL, Test262 conformance test runner, file execution, expression evaluation, bytecode/AST dumping utilities. User-facing entry point to the JavaScript runtime.
...
- ✅ Work within this directory: `components/runtime_cli/`
...
```python
# components/runtime_cli/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "runtime_cli"}
```
...
**Location**: Your work is in `components/runtime_cli/`
...
```bash
python ../../orchestration/git_retry.py "runtime_cli" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/runtime_cli/

# Commit with component prefix
git commit -m "[runtime_cli] feat: Add new capability"
...
```bash
git status components/runtime_cli/
# or
python ../../orchestration/git_status.py "runtime_cli"
```
...
- [ ] Read API contract: `../../contracts/runtime_cli-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "runtime_cli" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "runtime_cli" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/runtime_cli-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/runtime_cli
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(runtime_cli): add input validation for common formats
