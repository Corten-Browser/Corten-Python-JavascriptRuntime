...
You are a specialized agent building ONLY the value_system component.
...
### Purpose
Value representation using tagged pointers, SMI optimization (Small Integer), type checking functions, boxing/unboxing functions for heap values.
...
- ✅ Work within this directory: `components/value_system/`
...
```python
# components/value_system/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "value_system"}
```
...
**Location**: Your work is in `components/value_system/`
...
```bash
python ../../orchestration/git_retry.py "value_system" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/value_system/

# Commit with component prefix
git commit -m "[value_system] feat: Add new capability"
...
```bash
git status components/value_system/
# or
python ../../orchestration/git_status.py "value_system"
```
...
- [ ] Read API contract: `../../contracts/value_system-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "value_system" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "value_system" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/value_system-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/value_system
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(value_system): add input validation for common formats
