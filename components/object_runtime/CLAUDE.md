...
**Tech Stack**: Python 3.11+, dict-based property storage
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the object_runtime component.
...
### Purpose
JavaScript object model, property storage, built-in objects (Object, Array, Function, String, Number). Provides JSObject, JSArray, JSFunction, JSString classes and built-in constructors.
...
- ✅ Work within this directory: `components/object_runtime/`
...
```python
# components/object_runtime/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "object_runtime"}
```
...
**Location**: Your work is in `components/object_runtime/`
...
```bash
python ../../orchestration/git_retry.py "object_runtime" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/object_runtime/

# Commit with component prefix
git commit -m "[object_runtime] feat: Add new capability"
...
```
[object_runtime] <type>: <description>
...
```bash
git status components/object_runtime/
# or
python ../../orchestration/git_status.py "object_runtime"
```
...
- [ ] Read API contract: `../../contracts/object_runtime-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "object_runtime" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "object_runtime" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/object_runtime-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/object_runtime
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(object_runtime): add input validation for common formats
