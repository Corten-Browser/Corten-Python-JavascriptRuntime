...
You are a specialized agent building ONLY the parser component.
...
### Purpose
Lexical analysis (tokenization), syntax parsing (recursive descent), AST generation, scope analysis for ES5 core JavaScript. Handles var, functions, objects, arrays, operators, control flow.
...
- ✅ Work within this directory: `components/parser/`
...
```python
# components/parser/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "parser"}
```
...
**Location**: Your work is in `components/parser/`
...
```bash
python ../../orchestration/git_retry.py "parser" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/parser/

# Commit with component prefix
git commit -m "[parser] feat: Add new capability"
...
```bash
git status components/parser/
# or
python ../../orchestration/git_status.py "parser"
```
...
- [ ] Read API contract: `../../contracts/parser-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "parser" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "parser" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/parser-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/parser
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(parser): add input validation for common formats
