...
You are a specialized agent building ONLY the interpreter component.
...
### Purpose
Bytecode interpreter with dispatch loop, execution context management, call frames, stack management, runtime services (global variables, function calls, object operations). Executes bytecode and returns results.
...
- ✅ Work within this directory: `components/interpreter/`
...
```python
# components/interpreter/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "interpreter"}
```
...
**Location**: Your work is in `components/interpreter/`
...
```bash
python ../../orchestration/git_retry.py "interpreter" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/interpreter/

# Commit with component prefix
git commit -m "[interpreter] feat: Add new capability"
...
```bash
git status components/interpreter/
# or
python ../../orchestration/git_status.py "interpreter"
```
...
- [ ] Read API contract: `../../contracts/interpreter-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "interpreter" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "interpreter" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/interpreter-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/interpreter
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(interpreter): add input validation for common formats
