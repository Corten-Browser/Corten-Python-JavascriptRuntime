...
**Tech Stack**: Python 3.11+, tracemalloc for memory profiling
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the memory_gc component.
...
### Purpose
Memory allocation and simple mark-and-sweep garbage collection. Provides Allocator, GarbageCollector, HeapObject base class, and allocation functions.
...
- ✅ Work within this directory: `components/memory_gc/`
...
```python
# components/memory_gc/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "memory_gc"}
```
...
**Location**: Your work is in `components/memory_gc/`
...
```bash
python ../../orchestration/git_retry.py "memory_gc" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/memory_gc/

# Commit with component prefix
git commit -m "[memory_gc] feat: Add new capability"
...
```bash
git status components/memory_gc/
# or
python ../../orchestration/git_status.py "memory_gc"
```
...
- [ ] Read API contract: `../../contracts/memory_gc-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "memory_gc" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "memory_gc" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/memory_gc-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/memory_gc
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(memory_gc): add input validation for common formats
